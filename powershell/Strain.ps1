Function Invoke-Keep() {
    ...
    $KeepFilteredElements = @()

    foreach($Item in $Data) {
      if(& $Predicate $Item) {
        $KeepFilteredElements += ,$Item
      }
    }
    return($KeepFilteredElements)
}
...
Function Invoke-Discard() {
    ...
    $DiscardFilteredElements = @()

    foreach($Item in $Data) {
      if(!(& $Predicate $Item)) {
        $DiscardFilteredElements += ,$Item
      }
    }

    return($DiscardFilteredElements)
}